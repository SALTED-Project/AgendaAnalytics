...
command="$1"
projectname="salted_discoverandstore"

export $(grep '^MQTT_ENABLED' .env | xargs )
export $(grep '^MQTT_HOST' .env | xargs )
export $(grep '^SALTED_SEARCHENGINE_SERVICE' .env | xargs)

case "${command}" in
	"start")
        echo "Hello! Welcome to the start-up of this SALTED-service DiscoverAndStore ;D " 
        ...
        echo "Checking if necessary SALTED-SearchEngine is running." 
        fileserver_status=$(curl -s -w "%{http_code}" "${SALTED_SEARCHENGINE_SERVICE}/"| tail -c 3)
        echo ${fileserver_status} 
        if [ "${fileserver_status}" != "200" ] 
        then 
            echo "...it is not. Start up SALTED-SearchEngine first." 
            exit 127;
        else
            echo "... it is."
        fi 
        ...
        unset "MQTT_ENABLED"
        unset "MQTT_HOST"
        unset "SALTED_SEARCHENGINE_SERVICE"
        {   echo "First we clean up ..." \
            ...
            && docker-compose rm -s -f -v salted_discoverandstore_testdb \
            && echo "Removed testing db." \
            && echo "Webserver can be used now." 
            # ! can not start webserver here (see below), because redirection of logs to docker container logs does not work for the uvicorn command - therefor command in docker-compose.yml
            } || {
                echo "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!ERROR!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!" \
